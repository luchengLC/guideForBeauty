...
class Spider:
    ...
    # # 设置商品的类别
    # def getCategory(self,soup,item):
    #     div = soup.find('div', class_="crumb fl clearfix")
    #     kind = div.find_all('div', class_="item")
    #     item.first_category = kind[0].a.get_text()
    #     item.second_category = kind[2].a.get_text()
    #     item.third_category = kind[4].a.get_text()
    #     return item
    ...
    #获取参数详情
    def getParameter(self,soup, item):
        dt = soup.find('div', class_="Ptable").find('div', class_="Ptable-item").find_all('dt')
        dd = soup.find('div', class_="Ptable").find('div', class_="Ptable-item").find_all('dd')
        count = 0
        for i in dt:
            kind = i.get_text()
            value = dd[count].get_text()
            if kind.find('人群') >= 0:
                item.good_for_who = value
                continue
            if kind.find('保质期') >= 0:
    ...
    def getAttr(self,page,item,one,two,three):
        ...
        item = self.getParameter(soup,item)
